...
def write_donation_to_db(full, params, stripe_donation):
    """Note: stripe_donation can be the id of a charge or a subscription depending on params['recurring']"""
...
def get_donation_params(form):
    try:
        charges = list(map(int, filter(lambda a: a!="" and a!="other", form.getlist('charge[amount]'))))
        return (None, {
            'charge': charges[0],
...
def donation():
    ...
    (err, charge_id) = donate.widgets.stripe.create_charge(params['recurring'], params['stripe_token'], cents, params['email'])
    if err:
        return flash_donation_err(err)
